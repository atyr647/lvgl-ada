 ...
 *  Off: the SDL2 "dummy" video driver (SDL_VIDEODRIVER=dummy, used for
 *  headless test runs and CI) has no accelerated render driver, so
 *  SDL_CreateRenderer(..., SDL_RENDERER_ACCELERATED) fails and
 *  lv_sdl_window_create silently returns NULL -- every widget created
 *  against that NULL display then fails its own creation the same way
 *  (lv_obj_class_create_obj falls into its parent==NULL/no-default-display
 *  path). Software rendering works under both the dummy driver and a real
 *  one, so it is the safe default here; this binding's Lv.Sdl unit is
 *  documented as bring-up/testing-only anyway (SDLada is the production
 *  backend per FROZEN.md). */
